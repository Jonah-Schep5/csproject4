...
public class Bintree {
     ...
     * @param xwid Region x width
     * @param ywid Region y width
     * @param zwid Region z width
     * @param splitDim Split dimension (0=x, 1=y, 2=z)
     * @param result Array to store results: [leftX, leftY, leftZ, leftXwid, leftYwid, leftZwid,
     *               rightX, rightY, rightZ, rightXwid, rightYwid, rightZwid]
     */
    private static void calculateChildRegions(int x, int y, int z, int xwid, int ywid, int zwid,
        int splitDim, int[] result) {
        if (splitDim == 0) {
    ...
    private interface BintreeNode {
         ...
         */
        BintreeNode insert(AirObject obj, int x, int y, int z, int xwid,
            int ywid, int zwid, int depth, int splitDim);
         ...
         */
        void print(int x, int y, int z, int xwid, int ywid, int zwid,
            int depth, StringBuilder result, int[] nodeCount);
         ...
         *            StringBuilder to append results to
         */
        void findCollisions(int x, int y, int z, int xwid, int ywid, int zwid,
            int depth, StringBuilder result);
         ...
         */
        void findIntersections(int x, int y, int z, int xwid, int ywid,
            int zwid, int boxX, int boxY, int boxZ, int boxXwid, int boxYwid,
            int boxZwid, int depth, StringBuilder result, int[] nodeCount);
    }
    ...
    private static class EmptyLeafNode implements BintreeNode {
        ...
        @Override
        public BintreeNode insert(AirObject obj, int x, int y, int z, int xwid,
            int ywid, int zwid, int depth, int splitDim) {
        ...
        @Override
        public void print(int x, int y, int z, int xwid, int ywid, int zwid,
            int depth, StringBuilder result, int[] nodeCount) {
            nodeCount[0]++;
            result.append("E (");
            result.append(x);
        ...
        @Override
        public void findCollisions(int x, int y, int z, int xwid, int ywid,
            int zwid, int depth, StringBuilder result) {
        }

        @Override
        public void findIntersections(int x, int y, int z, int xwid, int ywid,
            int zwid, int boxX, int boxY, int boxZ, int boxXwid, int boxYwid,
            int boxZwid, int depth, StringBuilder result, int[] nodeCount) {
            nodeCount[0]++;
        }
    ...
    /**
     * Leaf node containing objects
     */
    private static class LeafNode implements BintreeNode {
        /**
         * Array of objects in this leaf (max 3, but can grow temporarily)
         */
        private AirObject[] objects;

        /**
         * Number of objects currently in this leaf
         */
        private int count;

        /**
         * Constructor for leaf node with one object
         *
         * @param obj
         *            The first object
         */
        ...
        public BintreeNode insert(AirObject obj, int x, int y, int z, int xwid,
            int ywid, int zwid, int depth, int splitDim) {
            if (!obj.intersects(x, y, z, xwid, ywid, zwid)) {
                return this;
            }
        ...
        /**
         * Check if all objects in this leaf intersect with each other
         *
         * @return True if all objects intersect
         */
        ...
        /**
         * Split this leaf node into an internal node
         *
         * @param x
         *            The x-coordinate of the region
         * @param y
         ...
         */
        private BintreeNode split(int x, int y, int z, int xwid, int ywid,
            int zwid, int depth, int splitDim) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (obj.intersects(regions[0], regions[1], regions[2], regions[3],
                    regions[4], regions[5])) {
                    internal.left = internal.left.insert(obj, regions[0], regions[1],
                        regions[2], regions[3], regions[4], regions[5], depth + 1,
                        nextDim);
                }
                if (obj.intersects(regions[6], regions[7], regions[8], regions[9],
                    regions[10], regions[11])) {
                    internal.right = internal.right.insert(obj, regions[6], regions[7],
                        regions[8], regions[9], regions[10], regions[11], depth + 1,
                        nextDim);
                }
            }
    ...
    private static class InternalNode implements BintreeNode {
        ...
        public void print(int x, int y, int z, int xwid, int ywid, int zwid,
            ...
            int[] regions = new int[12];
            calculateChildRegions(x, y, z, xwid, ywid, zwid, depth % 3, regions);
            left.print(regions[0], regions[1], regions[2], regions[3], regions[4],
                regions[5], depth + 1, result, nodeCount);
            right.print(regions[6], regions[7], regions[8], regions[9], regions[10],
                regions[11], depth + 1, result, nodeCount);
        }
        ...
        public void findIntersections(int x, int y, int z, int xwid, int ywid,
            ...
            if (!((x < boxX + boxXwid) && (x + xwid > boxX) && (y < boxY + boxYwid)
                && (y + ywid > boxY) && (z < boxZ + boxZwid) && (z + zwid > boxZ))) {
                return;
            }
            ...
            if ((regions[0] < boxX + boxXwid) && (regions[0] + regions[3] > boxX)
                && (regions[1] < boxY + boxYwid) && (regions[1] + regions[4] > boxY)
                && (regions[2] < boxZ + boxZwid) && (regions[2] + regions[5] > boxZ)) {
                left.findIntersections(regions[0], regions[1], regions[2], regions[3],
                    regions[4], regions[5], boxX, boxY, boxZ, boxXwid, boxYwid,
                    boxZwid, depth + 1, result, nodeCount);
            }

            if ((regions[6] < boxX + boxXwid) && (regions[6] + regions[9] > boxX)
                && (regions[7] < boxY + boxYwid) && (regions[7] + regions[10] > boxY)
                && (regions[8] < boxZ + boxZwid) && (regions[8] + regions[11] > boxZ)) {
                right.findIntersections(regions[6], regions[7], regions[8], regions[9],
                    regions[10], regions[11], boxX, boxY, boxZ, boxXwid, boxYwid,
                    boxZwid, depth + 1, result, nodeCount);
            }
        }
    }
     ...
     */
    public String findIntersections(int boxX, int boxY, int boxZ, int boxXwid,
        int boxYwid, int boxZwid) {
        StringBuilder result = new StringBuilder();
